...
class Scene_Map < Scene_Base
  ...
  def update
    super
    # 맵으로 돌아오는 경우 하드 난이도의 세이브 허용을 취소
  ...
  def create_location_window
    ...
    @map_name_window = Window_MapNamePlus.new
  end
  #--------------------------------------------------------------------------
...
=begin
  def update_call_menu
    ...
    #else
    # 메뉴 호출 오류 방지 추가
    #elsif !@interpreter and !$game_system.menu_disabled
    #  @menu_calling ||= Input.trigger?(:B)
    #  call_menu if @menu_calling && !$game_player.moving?
    #end
  end
...
=end
  ...
  def perform_battle_transition
    Graphics.transition(10, "Graphics/System/BattleStart", 100)
    Graphics.freeze
  end
  #--------------------------------------------------------------------------
  # * Get Fade Out Speed
