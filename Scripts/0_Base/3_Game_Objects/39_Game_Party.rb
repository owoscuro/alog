...
class Game_Party < Game_Unit
  ...
  # 효과음 죽이기 실험
  def gain_item_2(item, amount, include_equip = false)
    # Alog https://arca.live/b/alog
    # 새로운 아이템을 획득했다.
    $game_switches[58] = true
  ...
  def swap_order(index1, index2)
    # Alog https://arca.live/b/alog
    # 캐릭터 변경시 좌표 유지
    #@ro1_actor1_x = $game_player.x
    #@ro1_actor1_y = $game_player.y
    
    # 아래는 원본
    @actors[index1], @actors[index2] = @actors[index2], @actors[index1]
    
    # 캐릭터 변경시 좌표 유지
    #$game_player.x = @ro1_actor1_x
    #$game_player.y = @ro1_actor1_y
