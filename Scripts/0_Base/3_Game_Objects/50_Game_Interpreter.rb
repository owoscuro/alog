...
class Game_Interpreter
  ...
  def command_126
    ...
    if event != nil and event.name != nil
      rpg_enemy = $data_enemies[$1.to_i] if event.name =~ /<enemy: (.*)>/i
      if rpg_enemy != nil
        $game_party.bestiary.each do |entry|
        next unless entry.enemy_id == rpg_enemy.shown_id
          entry.dropitem = [] if entry.dropitem == nil
          if !entry.dropitem.include?([$data_items[@params[0]].name, $data_items[@params[0]].icon_index])
            entry.dropitem.push([$data_items[@params[0]].name, $data_items[@params[0]].icon_index])
            print("Game_Interpreter - %s \n" % [entry.dropitem]);
          end
        end
      end
    end
  ...
  def command_127
    ...
    if event != nil and event.name != nil
      ...
      if rpg_enemy != nil
        ...
        next unless entry.enemy_id == rpg_enemy.shown_id
          entry.dropitem = [] if entry.dropitem == nil
          if !entry.dropitem.include?([$data_weapons[@params[0]].name, $data_weapons[@params[0]].icon_index])
            entry.dropitem.push([$data_weapons[@params[0]].name, $data_weapons[@params[0]].icon_index])
            print("Game_Interpreter - %s \n" % [entry.dropitem]);
          end
        end
      end
  ...
  def command_128
    ...
    if event != nil and event.name != nil
      ...
      if rpg_enemy != nil
        ...
        next unless entry.enemy_id == rpg_enemy.shown_id
          entry.dropitem = [] if entry.dropitem == nil
          if !entry.dropitem.include?([$data_armors[@params[0]].name, $data_armors[@params[0]].icon_index])
            entry.dropitem.push([$data_armors[@params[0]].name, $data_armors[@params[0]].icon_index])
            print("Game_Interpreter - %s \n" % [entry.dropitem]);
          end
        end
      end
  ...
  def command_201
    ...
    # 맵 이동 가능 여부
    if $game_switches[143] == false and $game_party.members.size >= 2 and @map_id != 35
      screen.start_fadein(10)
      # 오류 메세지 표시 실험 -----------------------
      $game_temp.pop_w(180, 'SYSTEM', 
      "  동료가 전투중 혹은 너무 멀리 있습니다.  ")
      # -------------------------------------------
      return
    else
      Fiber.yield while $game_player.transfer? || $game_message.visible
      $game_player.direction_fix = true      # 방향 전환 고정
      ...
      # 조명 갱신
      $game_variables[155] = 99
      
      # 맵 이동시 특정 상태이상 제거
      $game_party.members.each do |actor|
        actor.remove_state(6) if actor.state?(6)    # 수면
        actor.remove_state(8) if actor.state?(8)    # 스턴
        actor.remove_state(11) if actor.state?(11)  # 석화
      end
      
      # 발사체 잔여물 제거, 투명 벽 오류 수정
      $game_player.projectiles.each do |projectile|
        if !projectile.destroy_it and projectile.draw_it = true
          $game_player.projectiles.delete(projectile)
        end
      end
    
      $game_player.reserve_transfer(map_id, x, y, @params[4])
      $game_player.direction_fix = false      # 방향 전환 고정 해제
      
      $game_temp.reserve_common_event(10)
      
      # 무조건 검은색 화면으로 화면 전환하도록 수정
  ...
  def command_231
    ...
    if @params[0] == 40 and $game_switches[148] == true
      # 기타 설정에서 고어 이미지 취소 추가
    else
      # NPC 스탠딩 이미지
      if $game_switches[283] == false and @params[0] == 22 and @params[6] == 70
        screen.pictures[@params[0]].show(@params[1], @params[2],
          x, y, $game_variables[83], $game_variables[83], @params[8], @params[9])
      elsif @params[0] == 40
        screen.pictures[@params[0]].show(@params[1], @params[2],
          x, y, @params[6] + 10, @params[7] + 10, @params[8], @params[9])
      else
        screen.pictures[@params[0]].show(@params[1], @params[2],
  ...
  def command_232
    if @params[3] == 0
      ...
      # NPC 스탠딩 이미지
      if $game_switches[283] == false and @params[0] == 22
        screen.pictures[@params[0]].move(@params[2], x, y, $game_variables[83],
          $game_variables[83], @params[8], @params[9], @params[10])
      elsif @params[0] == 2 and $game_map.map_id == 110
        screen.pictures[@params[0]].move(@params[2], x, y, $game_variables[83],
          $game_variables[83], @params[8], @params[9], @params[10])
      else
