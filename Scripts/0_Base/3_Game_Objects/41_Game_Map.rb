...
class Game_Map
  ...
  def setup(map_id)
    ...
    # 맵 이동중 내부, 동굴로 진입시 햇빛 효과 제거
    $game_map.screen.pictures[12].erase if !$game_map.screen.pictures[12].nil?
  end
  #--------------------------------------------------------------------------
  # * Create Vehicles
  #--------------------------------------------------------------------------
...
=end
  ...
  def tileset
    # Alog https://arca.live/b/alog
    # 45번 변수에 현 맵 타일셋 id 대입
    $game_variables[45] = @tileset_id
  ...
  def scroll_down(distance)
      ...
      @parallax_y += distance if @parallax_loop_y # 적용 완료
    #else
      last_y = @display_y
      @display_y = [@display_y + distance, height - screen_tile_y].min # 적용 완료
  ...
  def scroll_left(distance)
      ...
      #@display_x %= @map.width 
      @parallax_x -= distance if @parallax_loop_x # 적용 완료
    #else
      last_x = @display_x
      #@display_x = [@display_x - distance, -320].max # 적용 완료
  ...
  def scroll_right(distance)
    ...
    #else
      last_x = @display_x
      @display_x = [@display_x + distance, width - screen_tile_x].min # 적용 완료
      #@parallax_x += @display_x - last_x
    #end
  end
  #--------------------------------------------------------------------------
  ...
  def scroll_up(distance)
    ...
    #else
      last_y = @display_y
      #@display_y = [@display_y - distance, -320].max # 적용 완료
  ...
  #--------------------------------------------------------------------------
  # * 지정된 좌표에서 모든 레이어 타일의 배열 가져오기(위에서 아래로)
  #--------------------------------------------------------------------------
  ...
  def passable2?(x, y, d)
    if d == 2
      events_xy_nt_b(x, y + 1).count == 0
    elsif d == 8
      events_xy_nt_b(x, y - 1).count == 0
    elsif d == 4
      events_xy_nt_b(x - 1, y).count == 0
    elsif d == 6
      events_xy_nt_b(x + 1, y).count == 0
    end
  end
  def events_xy_nt_b(x, y)
    @events.values.select {|event| event.pos_nt?(x, y) and !event.enemy_ready?}
  end
  #--------------------------------------------------------------------------
