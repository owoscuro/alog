...
class Game_Character < Game_CharacterBase
  ...
  def init_private_members
    super
    @move_route = nil                 # 경로 이동
    @move_route_index = 0             # 경로 실행 위치 이동
    @original_move_route = nil        # 원래 이동 경로
    @original_move_route_index = 0    # 원래 이동 경로 실행 위치
    @wait_count = 0                   # 대기 횟수
  end
  #--------------------------------------------------------------------------
  # * Memorize Move Route
  ...
  def process_move_command(command)
    # 상태이상 이동속도 감소
    #@move_speed += battler.atk_times_add * 0.4 if battler != nil
    params = command.parameters if command.parameters != nil
    case command.code
