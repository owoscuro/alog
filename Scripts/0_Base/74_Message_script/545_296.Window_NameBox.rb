...
unless defined? Unicode
  ...
  #--------------------------------------------------------------------------
  module Unicode
    # Win32API remove: MultiByteToWideChar = Win32API.new('Kernel32','MultiByteToWideChar','llpipi','i')
    # Win32API remove: WideCharToMultiByte = Win32API.new('Kernel32','WideCharToMultiByte','llpipipp','i')
    UTF_8 = 65001
    def unicode!
      # Win32API remove: buf = "\0" * (self.size * 2 + 1)
      # Win32API remove: MultiByteToWideChar.call(UTF_8, 0, self, -1, buf, buf.size)
      # Win32API remove: buf
    end
    def unicode_s
      # Win32API remove: buf = "\0" * (self.size * 2 + 1)
      # Win32API remove: WideCharToMultiByte.call(UTF_8, 0, self, -1, buf, buf.size, nil, nil)
      # Win32API remove: buf.delete("\0")
    end
  end
  class String
    include Unicode
...
module IME
  ...
  # Win32API remove: CreateEdit = Win32API.new(DLL,'CreateEdit','ii','i')
  
  # 에디트 박스에 포커스를 설정한다
  # Win32API remove: SetIME = Win32API.new(DLL,'SetIME','i','i')
  
  # 에디트 박스를 제거하고 화면에 포커스를 설정한다
  # Win32API remove: ReleaseIME = Win32API.new(DLL,'ReleaseIME','v','i')
  
  # 에디트 박스 내의 전체 문장을 가져온다
  # Win32API remove: GetCharText = Win32API.new(DLL,'GetCharText','p','i')
  
  # 에디트 박스의 EditProc에서 마지막으로 조합된 한글 문자를 가져온다
  # Win32API remove: GetLastChar = Win32API.new(DLL, 'GetLastChar', 'v', 'p')
  
  # 에디트 박스의 EditProc에서 현재 조합 중인 한글 문자를 가져온다
  # Win32API remove: GetCompStr = Win32API.new(DLL, 'GetCompStr', 'v', 'p')
  
  # 특수 문자와 이스케이프를 포함한 문장의 폭을 가져올 수 있다
  # 하지만 폰트 선택이 되어있지 않기 때문에 폰트에 따라 폭이 달라지므로 부정확하다
  # Win32API remove: GetTextWidth = Win32API.new(DLL, 'GetTextWidth', 'pi', 'l')
  
  # 캐럿의 현재 인덱스를 가져온다
  # Win32API remove: GetCaretIndex = Win32API.new(DLL, 'GetCaretIndex', 'v', 'i')
  
  #--------------------------------------------------------------------------
  # * IME Setup
  #--------------------------------------------------------------------------
  def self.setup(max_char)
    # Win32API remove: CreateEdit.call(max_char,0)
  end
  #--------------------------------------------------------------------------
  # * 폭 (폰트 선택이 되어있지 않기 때문에 정확하지 않음)
  #--------------------------------------------------------------------------  
  def self.width(str, c)
    # Win32API remove: l = GetTextWidth.call(str.unicode!, c)
  end
  #--------------------------------------------------------------------------
  # * 한글 문자 체크
  #--------------------------------------------------------------------------    
...
module Locale
  # Win32API remove: GetUserDefaultLCID = Win32API.new('Kernel32', 'GetSystemDefaultUILanguage', 'v', 'l')
  # Win32API remove: GetUserDefaultLocaleName = Win32API.new('Kernel32', 'GetUserDefaultLocaleName', 'pl', 'l')
  
  @@locale = ""
  # Win32API remove: c = "\0" * 255
  # Win32API remove: GetUserDefaultLocaleName.call(c, 255)
  # Win32API remove: @@locale = c.delete("\0")
...
class Window_NameBox < Window_Base
  ...
  def create_item(actor, max_char)
    ...
    @max_char = max_char
    @buf = "\u0000" * 128
    # Win32API remove: @key_state = Win32API.new('user32','GetAsyncKeyState','i','i')
    # Win32API remove: IME::SetIME.call(@max_char)
  end
  #--------------------------------------------------------------------------
  # * 페이스칩 폭
  #--------------------------------------------------------------------------
  ...
  def left
    ...
    return [name_center - name_width / 2 + 20, contents_width - name_width].min
  end
  #--------------------------------------------------------------------------
  # * 글자의 폭
  #--------------------------------------------------------------------------
  ...
  #--------------------------------------------------------------------------
  def keyboard_check
    # Win32API remove: @handler.call if @key_state.call(0x0D) & 0x8000 != 0
  end
  #--------------------------------------------------------------------------
  # * 밑줄 그리기
  #--------------------------------------------------------------------------
  ...
  def refresh
    ...
    if @actor.id == 17 or @actor.id == 18
      draw_text(left - left / 3, line_height, contents.width, line_height, "지역의 이름을 입력하세요.")
    elsif @actor.id == 16
      draw_text(left - left / 3, line_height, contents.width, line_height, "집의 이름을 입력하세요.")
    elsif @actor.id == 15
      draw_text(left - left / 3, line_height, contents.width, line_height, "후원 코드를 입력하세요.")
    else
      draw_text(left - left / 3, line_height, contents.width, line_height, "아이의 이름을 입력하세요.")
    end
    @buf = "\u0000" * 128
    # Win32API remove: IME::GetCharText.call(@buf)
    draw_actor_face(@actor, 0, 0)
    # Win32API remove: @name = @buf.unicode_s.strip
    # Win32API remove: @index = IME::GetCaretIndex.call
...
class Scene_NameBox < Scene_MenuBase
  ...
  def prepare(actor_id, max_char)
    @actor_id = actor_id
    @max_char = max_char
    # Win32API remove: IME.setup(@max_char)
  end
  #-------------------------------------------------------------------------
  # * 시작
  #-------------------------------------------------------------------------
  ...
  def terminate
    super
    # Win32API remove: IME::ReleaseIME.call
  end
  #-------------------------------------------------------------------------
  # * 입력 완료
  #-------------------------------------------------------------------------
