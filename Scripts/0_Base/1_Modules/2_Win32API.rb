...
# https://github.com/mkxp-z/mkxp-z/blob/dev/scripts/preload/win32_wrap.rb
module RGU
	def self.raw_key_states(*args)
	  states = Array.new(512, false)
	  recent_triggered = Input.recent_pressed
...
def get_raw_keystates
	if $win32KeyStates == nil
		$win32KeyStates = RGU.raw_key_states
	end
...
def common_keystate(vkey)
	...
	if vkey_name == :LBUTTON
		pressed = Mouse.down?(Mouse::LEFT)
	elsif vkey_name == :RBUTTON
		pressed = Mouse.down?(Mouse::RIGHT)
	elsif vkey_name == :MBUTTON
		pressed = Mouse.down?(Mouse::MIDDLE)
	...
	module User32
		...
		class ShowCursor
			...
			def call(args)
				...
				Mouse.visible = @cursor_count >= 0
			end
		end

		class GetCursorPos
			def call(args)
				out = [Mouse.x, Mouse.y].pack('ll')
...
class Win32API
  NATIVE_ON_WINDOWS = false unless const_defined?("NATIVE_ON_WINDOWS")
  ...
  def initialize(dll, func, *args)
    ...
    if Win32API_Impl.const_defined?(dll)
      ...
      if dll_impl.const_defined?(func)
        @rgu_wrap_impl = dll_impl.const_get(func).new
      end
    end
  end
  ...
  def call(*args)
    if @rgu_wrap_impl
      return @rgu_wrap_impl.call(args)
    end

    if TOLERATE_ERRORS
      RGU::Console.puts("[Win32API] [#{@dll}:#{@func}] #{args.to_s}") if !@called
      @called = true
      return 0
    else
